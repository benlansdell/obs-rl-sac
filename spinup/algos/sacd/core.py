...
def placeholder_int(dim=None):
    return tf.placeholder(dtype=tf.int32, shape=(None,*dim) if dim else (None,))

def placeholders(*args):
    return [placeholder(dim) for dim in args]

def placeholders_int(*args):
    return [placeholder_int(dim) for dim in args]
...
def mlp_actor_critic_discrete(x, a, hidden_sizes=(100,10), activation=tf.nn.relu, 
    ...
    # policy
    with tf.variable_scope('pi'):
        mu, pi, logp_pi = policy(x, a, hidden_sizes, activation, output_activation, action_space)

        #Do we keep this??
        #mu, pi, logp_pi = apply_squashing_func(mu, pi, logp_pi)

    # make sure actions are in correct range
    #Do we keep this?? I don't think soo...
    #action_scale = action_space.high[0]
    #mu *= action_scale
    #pi *= action_scale
    ...
    x = tf.reshape(x, [-1, np.prod(x.shape[1:])])
    with tf.variable_scope('q1'):
        q1 = vf_mlp(tf.concat([x,a[:,None]], axis=-1))
    with tf.variable_scope('q1', reuse=True):
        q1_pi = vf_mlp(tf.concat([x,tf.cast(pi[:,None], tf.float32)], axis=-1))
    with tf.variable_scope('q2'):
        q2 = vf_mlp(tf.concat([x,a[:,None]], axis=-1))
    with tf.variable_scope('q2', reuse=True):
        q2_pi = vf_mlp(tf.concat([x,tf.cast(pi[:,None], tf.float32)], axis=-1))
    with tf.variable_scope('v'):
        v = vf_mlp(x)
    return mu, pi, logp_pi, q1, q2, q1_pi, q2_pi, v